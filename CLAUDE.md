...
| `dump_label` flags a name `spec_label` calls Bình thường | expected on 57.9% of dumping rows, and the reason the fifth component exists. His four detect a stock being pushed UP — PVDI divergence, abnormal turnover, wide range — and a crash is a different event. Measured on 13,111 crash sessions: his Volatility called 100.0% of them Bình thường, exactly, because a signed mean puts a fall at the BOTTOM of a distribution whose buckets only flag the top |
| `dump_label` = ban_thao_manh has POSITIVE forward returns (+2.05%) | not a bug, and the reason it is not in the rating penalty. Tail risk is monotone all the way — 2.76% / 4.59% / 8.02% / 8.83% chance of losing over 20% — but the extreme bucket bounces. It is a risk flag, not a sell signal |
| a defect-flagged bar still has `parkinson_21` but not `hl_range` | correct, and the distinction is the point. `panel.py` nulls open/high/low on suspect_move, band_anomaly and suspect_ohlc bars, so a SAME-SESSION range feature has nothing to compute from and is NULL. A 21-session estimator skips the nulled day and is built from the twenty good bars around it, which is why the bar is nulled rather than the row dropped. The first version of `defect_bars_have_no_range` checked both and failed on correct data |
